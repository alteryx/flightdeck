...
library(tidyverse)
glm(am ~ wt, mtcars, family="binomial") %>%
  tidy %>%
  fdTidyTable %>%
  fdPreview
...
tagList(
  fdInfoBox(adjRsquared, paste0(round(d$adj.r.squared*100, 1), '%'), 
    color = 'blue', icon = fdIcon('chart-line', lib = 'entypo')),
  fdInfoBox('F-Statistic', d$statistic,  'on 10 and 21 degrees of freedom', 
    color = 'green', icon = fdIcon('star', lib = 'entypo')),
  fdInfoBox('Residual Standard Error', d$deviance, 'on 21 degrees of freedom', 
    color = 'purple', icon = fdIcon('flash', lib = 'entypo'))
) %>%
  fdPreviewBoard(wrapBox = F)